...
def afficher_resultats(prob, u, x, d, r, y, camions, quais):
    ...
    def min_to_hhmm(m: float) -> str:
        """Convertit des minutes (depuis 18h00) en HHhMM."""
    ...
    line_width = len(SEP) - 4  # longueur utile pour le cadre
    ...
    for j in quais:
        ...
        print(f"  ┌─{titre}{'─' * (line_width - len(titre) - 1)}")
        print(HEADER)
        print(SEP)
        ...
        for i in camions_quai:
            ...
            Tl_i   = CAMIONS[i]["Tl"]
            Pi     = CAMIONS[i]["P"]
            attente = max(0.0, di - Ta_i)   # minutes d'attente avant début de chargement

            attente_str  = f"{int(round(attente))}m"
            retard_str   = f"{int(round(ri))}m"
            penalite_str = f"{int(round(Pi * ri))} ₮"

            print(
                f"  │  {i:<{W['camion']}}  "
                f"{CAMIONS[i]['transporteur']:<{W['trans']}}  "
                f"{CAMIONS[i]['S']:>{W['eco']}}  "
                f"{min_to_hhmm(Ta_i):>{W['arrivee']}}  "
                f"{min_to_hhmm(di):>{W['debut']}}  "
                f"{attente_str:>{W['attente']}}  "
                f"{min_to_hhmm(fin):>{W['fin']}}  "
                f"{min_to_hhmm(Tl_i):>{W['limite']}}  "
                f"{retard_str:>{W['retard']}}  "
                f"{penalite_str:>{W['penalite']}}"
            )
